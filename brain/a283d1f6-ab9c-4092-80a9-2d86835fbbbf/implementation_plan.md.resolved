...
### 2. Kiểm tra sec_uid
- **If Node**: Check xem `sec_uid` đã có chưa?
  - **Nếu CHƯA**: Gọi API `get_uniqueid`
    ...
    - Lưu `sec_uid` vào biến (UID đã có sẵn từ cột `ID TikTok`)
...
- **Get User Videos** (HTTP Request):
  ...
  - Query Parameters:
    - `uid`: `{{ $json["ID TikTok"] }}` (lấy từ cột có sẵn)
    - `sec_uid`: `{{ $json.sec_uid }}` (từ bước 2)
...
- **If**: Nếu có video mới
  ...
  - **Update Cấu Hình Đăng**: Cập nhật:
    - `sec_uid`: nếu mới lấy (UID đã có sẵn, không cần update)
    - `ID Post Video mới nhất`: `{{ $json.video_id }}`

## Cấu trúc Sheet cần thêm

### Sheet "Cấu Hình Đăng"
Thêm 2 cột mới (UID/ID TikTok đã có sẵn):
- `sec_uid` (column mới)
- `ID Post Video mới nhất` (column mới)
