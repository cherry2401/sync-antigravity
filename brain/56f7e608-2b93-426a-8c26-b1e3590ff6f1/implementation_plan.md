...
- Add `Product` model with fields:
    - `name`, `slug` (unique), `sku`, `brand`, `category`
...
## Category & Brand CRUD Implementation 🏷️
### Database Schema
#### [MODIFY] [schema.prisma](file:///i:/Website/MyphamHaKi/app-src/prisma/schema.prisma)
- Add `Category` and `Brand` models to support dynamic management.
- Update `Product` to relationally link to these models (optional but good for integrity) or keep string for now and migrate later. *Adhering to user request: pure models first.*
...
- Replicate `ProductForm` image upload logic (Cloudinary).
- simple form: Name + Image.
- On success: `toast` + `router.refresh()`.

## Wishlist Implementation ❤️
### Database Schema
#### [VERIFY] [schema.prisma](file:///i:/Website/MyphamHaKi/app-src/prisma/schema.prisma)
- Ensure unique constraint on `[userId, productId]` to prevent duplicate likes.

### Server Actions
#### [NEW] [src/actions/wishlist.ts](file:///i:/Website/MyphamHaKi/app-src/src/actions/wishlist.ts)
- `toggleWishlist(productId)`: Authenticated user only. Checks existence -> Create/Delete. Revalidate paths.
- `getWishlist()`: Return list of product IDs (for button state) or full products (for page).
...
- Client component.
- Accepts `productId` and `initialIsLiked`.
- Optimistic updates using `useOptimistic` or local state with rollback.
...
- Reuse `ProductCard`. appears on storefront.
