# Walkthrough - Watcher Stability & AppData Migration 🚁✅
...
### 1. AppData Migration (Vite HMR Isolation) 🛡️
All communication files (`inbound_chat.json`, `outbound_chat.json`, `cdp_inbound.json`, `cdp_outbound.json`, `chopper_logs.json`) have been moved from the project workspace to the user's local AppData folder.
- **Electron Main**: Exposed `system:getCommDir` to provide the `userData` path.
- **Preload**: Added `getCommDir` to the `electronAPI` interface.
- **Refactor**: All components (`App`, `ChatPanel`, `ChopperLogs`) and services (`BrowserRelay`) now fetch and use this dynamic path.

### 2. Global Watcher Stability 🚁
The inbound message watcher is now hosted in the top-level `App.tsx`.
...
### 3. Extension Auto-Reconnect 🔌
Modified the Chrome extension's `background.js` to proactively check its connection to the App's relay server.
...
- **Documentation**: Updated `CHOPPER_GUIDE.md` with instructions on where to find the new communication files.
