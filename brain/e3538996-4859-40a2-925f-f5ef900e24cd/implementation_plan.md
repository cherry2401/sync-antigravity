...
2.  **Code Integration**:
    ...
    *   Inject tracking calls into `HomeVideoDownloader.tsx`, `AiWriter.tsx`, `ShopeeAffiliate.tsx`.
...
### Phase 3: In-App Admin Dashboard (Visual Reports)
1.  **Route**: Create a protected page `/admin-stats` (password protected via simple local storage or hardcoded env for now).
2.  **UI**:
    *   **Summary Cards**: Total Views, Total Downloads, Redirect Ratio.
    *   **Charts**: Line chart showing Downloads vs Shopee Clicks over last 7 days.
    *   **Table**: Recent activity log.
3.  **Function**: Fetches data directly from `analytics_events` table and aggregates it.
#### Connection Diagnostic
#### [MODIFY] [AdminDashboard.tsx](file:///src/components/AdminDashboard.tsx)
- Add "Connection Diagnostic" button.
- Clean up UI (Password visibility, Filters).
### Connection Check
- Implement logic to test Supabase connection and Insert permissions.
- Display explicit error messages (Missing Keys, RLS blocked, Network error).

## Reporting
*   **Immediate**: You can view the Admin Page on your website.
*   **Deep Dive**: Use Supabase Dashboard for complex SQL queries if needed.
